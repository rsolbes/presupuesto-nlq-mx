...
Generado el 2026-09-18 09:44 por `tests/verificar_rol_consulta.py`, conectado como `consulta_nlq` a `presupuesto_nlq`.
...
| no escribe aun sin solo lectura | barrera | SQLSTATE `42501` | SQLSTATE `42501` | paso | los privilegios detienen la escritura aunque se desactive el barandal |
...
| solo lectura por defecto | barandal | valor `on` | valor `on` | paso | toda transaccion inicia en solo lectura |
| escritura bloqueada por defecto | barandal | SQLSTATE `25006` | SQLSTATE `25006` | paso | el barandal detiene la escritura antes que los privilegios |
| tiempo limite configurado | barandal | valor `15s` | valor `15s` | paso | limite por sentencia |
| tiempo limite efectivo | barandal | SQLSTATE `57014` | SQLSTATE `57014` | paso | una sentencia de 20 s se cancela a los 15 s |
